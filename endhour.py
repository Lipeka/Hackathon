...
class SmartBoard:
    ...
    def schedule_hourly_alerts(self):
        ...
        target_hours = [9, 10, 11, 12, 13, 14, 15]
        for hour in target_hours:
            if current_hour == hour and current_time <= current_time.replace(hour=16):
                time_difference = (current_time.replace(hour=hour) - current_time).total_seconds()
    ...
    def update_current_hour(self):
        ...
        if current_day in self.smart_board.schedule:
            ...
            if 8 <= current_hour < 16:
                ...
                hour_interval = (current_hour - 8) // 2 + 3
